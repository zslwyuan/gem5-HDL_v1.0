...
FpgaCPU::activateContext(ThreadID thread_num)
{
    ...
    if (fpid == 0)
    {
		string tmp = "./fpga/" + moduleName;
        int rc=execl(tmp.c_str(),moduleName.c_str(),(char*)0);
		tmp.~string();
		if (rc == -1)
                          ...
                          bool read)
{
	...
	//else
	//	pkt = buildPacket(req, MemCmd::WriteReq);

	pkt->setHasSharers();
	//if (!pkt->cacheResponding()&&noL1)
...
FpgaCPU::fetch()  //FPGACPU-special==========================actually FPGA has no procedure of fetching but it can as a procedure of communicating with another process of verilator
{
    ...
    if ((ReadEnable && (outputArray[bit_FinishRead] || edge_RENA)) || readFault)
    {
		if (readFault)
			fatal("readFault");
		readFault = 0;
        ...
        inputArray[bit_ReadReady] = 0;
		if (noL1)
			initiateMemRead(outputArray[bit_ReadAddr], outputArray[bit_Size_ReadData_Output],Request::UNCACHEABLE);
		else
    ...
    if ((WriteEnable && (outputArray[bit_FinishWrite] || edge_WENA)) || writeFault)
    {
		writeFault = 0;
        WriteReady = 0;
        inputArray[bit_WriteReady] = 0;
		...
		if (noL1)
			 writeMem((uint8_t*)&outputArray[bit_WriteData], outputArray[bit_Size_WriteData],
                 outputArray[bit_WriteAddr], Request::UNCACHEABLE, NULL);
		else
        	 writeMem((uint8_t*)&outputArray[bit_WriteData], outputArray[bit_Size_WriteData],
...
//	printf("val    %lu\n",val);
    ...
    switch (regid)
    {
		case 0: if (!OccupyFPGA&&val) {TaskHash=val;OccupyFPGA=1;}
				break;
        ...
        case 6: RunState = val;inputArray[bit_Run] = (val==0); if(!RunState) printf("Reset FPGA @ tick %ld\n",curTick());break;
